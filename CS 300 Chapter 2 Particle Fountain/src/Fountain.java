...
/**
 * The Fountain Class contains method for a fully functioning Particle Fountain program. It also
...
/**
 * The Particle Fountain class contains methods to contribute to creating an image with numerous
 * particles/pixels. The supporting methods are contained in this class and called from the Utility
 * package.
 * 
 */
public class Fountain {
  ...
  /**
   * Main method used to run the Particle Fountain program and call the necessary supporting
   * methods.
   * 
   * @param args
   */
  ...
  /**
   * The setup method is used to initialize all of the private static fields. It sets all of the
   * default values for the particle fountain program. It also calls the two testing methods and
   * prints out failed if either of the two tests fail.
   * 
   */
  ...
  /**
   * The update method is used to update the particles by calling the various methods. It serves as
   * the main driver for the particle fountain program. It constantly iterates through the particles
   * array updating their position.
   * 
   */
  ...
  /**
   * The updateParticle method is used to update the position of the particles. This method is
   * called and then updates the position of the particle as gravity has an effect on it.
   * 
   * @param index The index of the particle in the particles array to update.
   * 
   */
  ...
  /**
   * The createNewParticles method is used to create new Particles in the particles array. The
   * parameter number determines the number of particles the method will create. The method iterates
   * through the particles array looking for null indexes. If it finds an index that is null it
   * creates a new particle setting all of its attributes to random values. The method ends after it
   * iterates through the entire particles array or when the correct number of particles have been
   * created.
   * 
  ...
  private static void createNewParticles(int number) {
    ...
    for (int i = 0; i <= particles.length - 1; i++) {
      if (number > 0 && particles[i] == null) {
        ...
        // Fountain.positionX.
        particles[i]
            .setPositionX(Fountain.positionX + (randGen.nextFloat() + 2) - (randGen.nextInt(6)));

        // The y position of each particle should begin within 3 pixels (+/-) of
        // Fountain.positionY.
        particles[i]
            .setPositionY(Fountain.positionY + (randGen.nextFloat() + 2) - (randGen.nextInt(6)));
  ...
  /**
   * The removeOldParticles method is used to remove particles in the particles array that are over
   * a specified age. This age is determined by the parameter maxAge. This method iterates through
   * the particles array and if the age is over maxAge then that index is set to null and if the age
   * is under or equal to max age the particle is left alone.
   * 
   * @param maxAge The maximum age for a particle before it is removed.
   * 
   */
  ...
  /**
   * The mousePressed method is used to change the location of the fountain when the user clicks the
   * mouse somewhere on the screen. Constantly updating the fountain's position as the when the user
   * presses the mouse.
   * 
   */
  ...
  /**
   * The keyPressed method is used to take a screen shot of the particle fountain and save it when
   * the user presses 'p' on the keyboard.
   * 
   */
  ...
  private static boolean testUpdateParticle() {
    ...
    // it looked better
    particles[0]
        .setColor(Utility.lerpColor(Fountain.startColor, Fountain.endColor, randGen.nextFloat()));
