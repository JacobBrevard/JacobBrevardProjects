...
public class Subscriber {
  ...
  private final static int CARD_BAR_CODE_INIT = 2019000001;

  // last card bar code that can be assigned
  private final static int CARD_BAR_CODE_LAST = 2019999999;

  // maximum number of books to be checked out one subscriber
  private final static int MAX_BOOKS_CHECKED_OUT = 10;


  // class variable that represents the card bar
  private static int nextCardBarCode = CARD_BAR_CODE_INIT;
  ...
  // 4-digits Personal Identification Number to verify the identity of this subscriber. The most
  // significant digit (the digit at the 4th position from the right) MUST NOT be zero
  private int pin;
  ...
  public void checkoutBook(Book book) {
    if (book.isAvailable()) // check if the book is available
      if (booksCheckedOut.size() < MAX_BOOKS_CHECKED_OUT) { // check if the subscriber did not
                                                            // already
  ...
  public void returnBook(Book book) {
    ...
    if (isBookInBooksCheckedOut(book)) {
      ...
      System.out
          .println("Sorry, you cannot return this book. It is not in your booksCheckedOut list.");
  }
